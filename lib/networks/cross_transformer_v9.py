...
class Network(nn.Module):
    def __init__(self):
        ...
        if cfg.camera_embedding:
            print('add camera embedding')
            self.latent = nn.Embedding(23, 128)
        ...
        self.latent_fc = nn.Conv1d(256+128, 256, 1)
    ...
    def forward(self, pixel_feat, sp_input, grid_coords, viewdir, light_pts,
        ...
        latent = self.latent(cam_id)
        features = torch.cat([features, latent[..., None].expand(*latent.shape, features.size(2))], dim=1)
        features = self.actvn(self.latent_fc(features))
