...
class Renderer:
    ...
    def get_pixel_aligned_feature(self, batch, xyz, pixel_feat_map,
        ...
        if cfg.run_mode == 'train' and cfg.perturb_pixel > 0:
            shift_pixel = torch.normal(0, cfg.std, size=uv.size()).to(uv.device)
            uv += cfg.perturb_pixel * shift_pixel
            uv = torch.clamp(uv, min=0, max=image_shape[0])
    ...
    def transform_pts2frames(self, pbw, pts, batch, debug=False, batchify=False, chunk=1024 * 32):
        # function to transform the points from target space to the different observation frames
    ...
    def render(self, batch):
        ...
        # get the SMPL based blending weights for pts
        pbw = bounds_grid_sample_blend_weights(pts.view(pts.shape[0], -1, 3), bw, batch['bounds'])
        n_batch = pbw.shape[0]
        n_point = pbw.shape[-1]
        # filter points that are far away from the surface
        pnorm = pbw[:, -1]
        ...
        pts = pts[pind][None]
        # refine blending weights with intial blending weights, distance to the surface and pose as the condition
        if cfg.bw_refine:
        ...
        # transform the target points to the observation space
        ...
        if ray_o.size(1) <= 2048:
            # get features from observed frames
