...
class Evaluator:
    ...
    def ssim_metric(self, rgb_pred, rgb_gt, batch, depth_map=None):
        ...
        if depth_map is not None:
            ...
            depth_pred[mask_at_box] = depth_map
            depth_pred = depth_pred[y:y + h, x:x + w]
        ...
        if depth_map is not None:
            depth_dir = os.path.join(human_dir, 'depth')
            # cv2.imshow('depth_pred', depth_pred)
            # cv2.waitKey()
            if not os.path.exists(depth_dir):
                os.makedirs(depth_dir)
            cv2.imwrite('{}/frame{}_view{}.png'.format(depth_dir, frame_index, view_index), np.uint8(depth_pred * 255))
    ...
    def evaluate(self, output, batch):
        ...
        # depth_map = batch['depthsup'][0, :, 0].detach().cpu().numpy()
        ...
        ssim = self.ssim_metric(rgb_pred, rgb_gt, batch, depth_map=None)
