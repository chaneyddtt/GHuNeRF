...
class Trainer(object):
    def __init__(self, network):
        device = cfg.local_rank # torch.device('cuda:{}'.format(cfg.local_rank))
    ...
    def val(self, epoch, data_loader, evaluator=None, recorder=None):
        ...
        for batch in tqdm.tqdm(data_loader):
            batch = self.to_cuda(batch)
            with torch.no_grad():
                output, loss, loss_stats, image_stats = self.network(batch, True)
            ...
            if iter_num % 50 == 0:
                ...
                for k, v in image_stats.items():
                    file_name = pattern + k + str(iter_num)
                    recorder.writer.add_image(file_name, np.transpose(v, (2, 0, 1)))
            iter_num += 1
