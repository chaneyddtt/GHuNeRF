...
def get_near_far(bounds, ray_o, ray_d):
    ...
    # print(mask_at_box.shape, np.count_nonzero(mask_at_box))
    return near, far, mask_at_box


def get_near_far_wdepth(bounds, ray_o, ray_d, depth):
    """calculate intersections with 3d bounding box"""
    bounds = bounds + np.array([-0.01, 0.01])[:, None]
    nominator = bounds[None] - ray_o[:, None]
    # calculate the step of intersections at six planes of the 3d bounding box
    ...
    depth = depth[mask_at_box]
    depth_map_near = depth - cfg.depth_range
    depth_map_far = depth + cfg.depth_range
    depth_mask = (depth > -1).astype(np.float32)
    depth_map_near = depth_map_near * depth_mask
    depth_map_far = depth_map_far * depth_mask

    near_com = np.maximum(near, depth_map_near)
    far_com = np.minimum(far, depth_map_far)
    near = np.where(depth_mask, near_com, near)
    far = np.where(depth_mask, far_com, far)
