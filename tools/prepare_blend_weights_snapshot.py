...
def get_transform_params(smpl, params):
    ...
    R = cv2.Rodrigues(params['pose'][:3])[0]
    Th = params['trans'][None]
...
def get_bweights(params, vertices_path):
...
def prepare_blend_weights(begin_frame, end_frame, frame_interval):
    ...
    params_all = np.load(param_path, allow_pickle=True).item()
    params = dict.fromkeys(params_all.keys())
    params['beta'] = params_all['beta']
    for i in tqdm(range(begin_frame, end_frame, frame_interval)):
        for k in ['pose', 'trans']:
            params[k] = params_all[k][i]
        vertices_path = os.path.join(vertices_dir, '{}.npy'.format(i))
        bweights = get_bweights(params, vertices_path)
...
parser.add_argument('--data_dir', type=str, default='data/people_snapshot')
parser.add_argument('--humans', type=str, nargs='+', default=['female-3-casual'])
...
for human_ind in tqdm(range(len(args.humans))):
    ...
    try:
        ...
        param_path = os.path.join(args.data_dir, human, 'params.npy')
        ...
        smpl_path = os.path.join('data', 'smpl_model', model_path)
        ...
        last_frame = len(os.listdir(vertices_dir)) + begin_frame
